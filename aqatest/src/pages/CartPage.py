...
from aqatest.src.pages.locators.CartPageLocators import CartPageLocators
...
class CartPage(CartPageLocators):
    ...
    def get_displayed_massage(self):
        txt = self.sl.wait_and_get_text(self.CART_PAGE_MASSAGE)
        return txt

    def apply_coupon(self, coupon_code):
        self.input_coupon(coupon_code)
        self.click_apply_coupon()
        success_msg = self.get_displayed_massage()
        assert success_msg == 'Coupon code applied successfully.', f'Unexpected massage when apply coupon'
